...
def main():
    ...
    is_night = False
    pregunta_alarma = input("Desea configurar una alarma?(S/N): ").upper()
    if pregunta_alarma == "S":
        hora_alarma = int(input("Introduce la hora (Formato 24h HH): "))
    while True:
        sleep(HOUR_DURATION)
        current_time += datetime.timedelta(hours=1)
        light_changed = False
        if hora_alarma == current_time.hour:
            write_file_and_screen("la alarma ha sonado", "horas.txt")
